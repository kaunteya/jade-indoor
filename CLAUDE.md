...
A small static site (Jade Indoor Sports Festival) with no build step and no dependencies — every page runs directly in the browser. Two files at the root are shared: `games.js` (the category list, loaded by every page) and `draws.js` (the `schedules/data/` file list and CSV reader, loaded by `schedules/`, `search/` and — for `loadLastUpdated()` alone — the landing page). Each page otherwise owns its own HTML/CSS/JS:

- **`index.html` + `home.css`** — the landing page, three stacked destination cards (Participants list, Schedule, Search) above a plain "Registrations closed" line. There is no Registration card: entries have closed, so it was removed rather than shown inert — the page is still deployed at `/registration`, just unlinked. Re-enable it by adding an `<a class="home-card" href="registration/">` back to `.home-nav` and dropping the `.closed-note`. The Schedule card's note says when the schedule was last updated (`Last updated on Sun 16 Aug, 1:18 pm`) and nothing else — it replaced the "Match times, courts and draws" description, which the other two cards still carry. No date is written down for it: `home.js` asks `loadLastUpdated()` in `draws.js` for the newest `Last-Modified` across `schedules/data/`, read with HEAD requests, so recording a result is all it takes to move the line. A server that sends no `Last-Modified` — or `file://`, which sends no headers at all — leaves the line hidden rather than showing a wrong date, so the card is title-only when the page is opened off the filesystem.
