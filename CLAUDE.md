...
- Doubles entries render a `<game_id>_partner` name input under their checkbox. No extra sheet column: a doubles game's own column holds the partner's *name* instead of a boolean (empty string when that game wasn't picked), so the column count still matches `GAME_FIELDS` one-to-one. `Code.gs` detects doubles by `'doubles'` appearing in the id, so there's no second list to sync — but a doubles id not containing `doubles` would silently be written as a boolean.
