...
3. **`#section-payment`** ("Payment") — UPI link, QR code, an optional UTR/Transaction ID input, a required payment-screenshot file input, and the Submit button. Revealed by `updateTotalCost()` once the running total is above zero; the Submit button inside it stays hidden until a screenshot is attached. The image is downscaled client-side (`resizeToDataUrl()`) and sent as a base64 field; `Code.gs` decodes it, saves it privately to a Drive folder, and adds a link + floating thumbnail to the row.
