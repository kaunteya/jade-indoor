...
- Doubles entries render a `<game_id>_partner` name input under their checkbox, written to its own sheet column immediately after that game's column. `Code.gs` detects doubles by `'doubles'` appearing in the id, so there's no second list to sync — but a doubles id not containing `doubles` would silently lose its partner column.
